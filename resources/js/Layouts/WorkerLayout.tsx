            ...
            {isMobileMenuOpen && (
                <div className="md:hidden glass border-b border-slate-200/50 dark:border-slate-800/50 absolute top-20 left-0 w-full z-40 shadow-xl slide-in-bottom">
                    <nav className="flex flex-col p-4 gap-2">
                        ...
                        <Link
                            href={route('data-requests.index')}
                            className={`p-4 rounded-xl flex items-center gap-3 font-medium ${route().current('data-requests.index') ? 'bg-primary/10 text-primary' : 'text-slate-600 hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-slate-800'}`}
                            onClick={() => setIsMobileMenuOpen(false)}
                        >
