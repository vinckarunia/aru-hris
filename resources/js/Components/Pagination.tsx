...
interface Props {
    ...
    /** Optional callback fired when the user changes the items-per-page value. */
    onItemsPerPageChange?: (perPage: number) => void;
    /** Available per-page options. Defaults to [10, 25, 50, 100]. */
 ...
 * Optionally renders a per-page selector dropdown when onItemsPerPageChange is provided.
 *
 * @param {Props} props - The component props.
 * @returns {JSX.Element | null} The rendered pagination bar, or null if not needed.
 */
export default function Pagination({ totalItems, itemsPerPage, currentPage, onPageChange, onItemsPerPageChange, perPageOptions = [10, 25, 50, 100] }: Props) {
    const totalPages = Math.ceil(totalItems / itemsPerPage);

    // Always show the bar when onItemsPerPageChange is provided (so user can change per-page even with 1 page)
            ...
            <div className="flex items-center gap-3 order-2 sm:order-1">
                {onItemsPerPageChange && (
                    <select
                        ...
                        className="text-sm border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300 rounded-lg py-1 pl-2 pr-7 focus:border-primary focus:ring-primary"
