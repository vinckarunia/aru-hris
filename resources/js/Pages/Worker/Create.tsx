                ...
                {isPic && (
                    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                            ...
                            <div>
                                <InputLabel htmlFor="hire_date">Tanggal Bergabung {data.contract_type === 'Kontrak' && <span className="text-red-500 font-bold ml-1">*</span>}</InputLabel>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
                        ...
                        <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                            ...
                            <div>
                                ...
                                <select id="pkwt_type" className="mt-1 block w-full rounded-md border-slate-300 dark:bg-slate-900 dark:border-slate-700 disabled:opacity-50" value={data.pkwt_type} onChange={e => setData('pkwt_type', e.target.value)} disabled={data.contract_type !== 'Kontrak'}>
                                    <option value="" disabled={data.contract_type === 'Kontrak'}>{data.contract_type !== 'Kontrak' ? data.contract_type : 'Pilih...'}</option><option value="PKWT">PKWT</option><option value="PKWTT">PKWTT</option>
                                </select>
                                <InputError message={(errors as any).pkwt_type} className="mt-1" />
                            </div>
                            <div>
                                <InputLabel htmlFor="pkwt_number" value="PKWT Ke-" />
                                <TextInput id="pkwt_number" type="number" className="mt-1 block w-full disabled:opacity-50" value={data.pkwt_number} onChange={e => setData('pkwt_number', e.target.value)} disabled={data.contract_type !== 'Kontrak' || data.pkwt_type === 'PKWTT'} placeholder="Opsional — Contoh: 1" />
                            ...
                            <div>
                                ...
                                <TextInput id="start_date" type="date" className="mt-1 block w-full" value={data.start_date} onChange={e => setData('start_date', e.target.value)} required={data.contract_type === 'Kontrak'} />
                                <InputError message={(errors as any).start_date} className="mt-1" />
                            </div>
                            <div>
                                <InputLabel htmlFor="end_date" value="Tanggal Berakhir Kontrak" />
                                <TextInput id="end_date" type="date" className="mt-1 block w-full disabled:opacity-50" value={data.end_date} onChange={e => { lastEditedBy.current = null; setData('end_date', e.target.value); }} disabled={data.contract_type !== 'Kontrak' || data.pkwt_type === 'PKWTT'} />
                                <InputError message={(errors as any).end_date} className="mt-1" />
                            </div>
                            <div>
                                <InputLabel htmlFor="duration_months" value="Durasi (Bulan)" />
                                <TextInput id="duration_months" type="number" className="mt-1 block w-full disabled:opacity-50" value={data.duration_months} onChange={e => { lastEditedBy.current = null; setData('duration_months', e.target.value.replace(/\D/g, '')); }} disabled={data.contract_type !== 'Kontrak' || data.pkwt_type === 'PKWTT'} placeholder="Contoh: 3" min="1" />
