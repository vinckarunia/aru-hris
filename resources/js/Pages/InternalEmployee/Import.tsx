 ...
 */
export default function Import({ dbColumns, autoMapHints }: Props) {
    // ---- Step Management ----
    ...
    const handleFileUpload = useCallback(async (file: File) => {
        ...
        setIsUploading(true);
        try {
            const response = await axios.post(route('internal-employees.import.upload'), formData);
    ...
    const handleValidate = async () => {
        ...
        try {
            const response = await axios.post(route('internal-employees.import.validate'), {
    ...
    const handleProcess = async () => {
        ...
        try {
            await axios.post(route('internal-employees.import.process'), {
                session_id: sessionId,
                mapping,
    ...
    const startProgressPolling = () => {
        ...
        progressIntervalRef.current = setInterval(async () => {
            if (!sessionId) return;
            try {
                const response = await axios.get(route('internal-employees.import.progress', { sessionId }));
    ...
    return (
        <AdminLayout title="Import Data Karyawan Internal" header="Import Data">
            {/* Back Button */}
            <div className="mb-4">
                <Link
            ...
            {currentStep === 1 && (
                ...
                >
                    ...
                    <div className="flex gap-3">
                        ...
                        <a
                            href={route('internal-employees.import.template')}
                                            ...
                                            return (
                                                <React.Fragment key={r.row_number}>
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-4 py-3 text-xs">{(r.preview.hire_date || r.preview.join_date) || '-'}</td>
                                                        <td className="px-4 py-3">
                                                            {r.errors.length > 0 ? (
                                ...
                                {progress.status === 'completed' && (
                                    <div className="flex justify-center gap-3">
                                        {progress.failed > 0 && progress.failed_file_path && (
                                            <a
                                                href={route('internal-employees.import.download-failures', { sessionId })}
