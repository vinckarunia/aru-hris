...
/**
 * Internal Employee Create Page Component
 *
 * Provides a comprehensive form to register a new internal employee of PT. ARU.
 */
export default function Create() {
    const { data, setData, post, processing, errors } = useForm({
        ...
        position: '', department: '', join_date: '', status: 'active',
    });
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('internal-employees.store'));
    };

    return (
        <AdminLayout title="Tambah Karyawan Internal" header="Karyawan Internal">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h2 className="text-xl font-bold text-slate-800 dark:text-white">Tambah Karyawan Internal</h2>
                    <p className="text-sm text-slate-500">Silakan lengkapi form pendaftaran karyawan internal PT. ARU di bawah ini.</p>
                </div>
            ...
            <form onSubmit={submit} className="space-y-6">
                ...
                <div className="flex justify-end gap-4 pb-10">
                    <Link href={route('internal-employees.index')} className="px-6 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-300 rounded-xl font-bold transition-colors">
                        Batal
                    </Link>
                    <PrimaryButton disabled={processing} className="px-8 py-2 rounded-xl text-base bg-primary hover:bg-primary-dark text-white font-bold shadow-lg shadow-primary/30 transition-all">
                        {processing ? 'Menyimpan...' : 'Simpan Data Karyawan Internal'}
                    </PrimaryButton>
                </div>
            </form>
        </AdminLayout>
